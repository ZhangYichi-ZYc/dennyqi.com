...
首先考虑以最坏情况的时间步数作为时间复杂度定义的复杂性类划分方式。对于概率图灵机$\P$，如果时间函数$T$满足$\forall x$，$\P(x)$计算成功的所有可能计算步数都不超过$T(|x|)$，就称$T$是$\P$的时间函数。如果$\P(x)$能在$cT(n)$内判定$L$，就称$L\in \text{PTIME}(T(n))$。
...
对于问题$L$和概率图灵机$\P$，如果时间函数$T$满足$\forall x$，$\P(x)$以单侧误差计算成功的所有可能计算步数的最坏步数不超过$cT(|x|)$，就称$L\in \text{RTIME}(T(n))$。其中，单侧误差计算成功定义为，$\forall x$，如果$x\not\in L$则$\Pr[\P(x)=1]=0$；如果$x\in L$则$\Pr[\P(x)=1]\geq 2/3$。定义$\text{RP}=\bigcup\limits_{c>0}\text{RTIME}(n^c)$，称为单侧误差概率多项式类或随机多项式类(Randomized Probabilistic Polynomial time complexity class)。
