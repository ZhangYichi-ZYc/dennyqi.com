$\newcommand{\I}{\mathfrak{I}}\newcommand{\A}{\mathfrak{A}}$我们已经说明了基于一阶逻辑语言的相继式演算具有可靠性(soundness)，也即：选定符号集$S$，对于任意$S$-formula集合$\Phi$和$S$-formula $\varphi$，如果$\Phi\vdash \varphi$成立，那么一定有$\Phi \models \varphi$成立。可靠性是简单而直接的，我们只是把定义展开就得出了结论。更困难的反过来的问题。我们想证明基于一阶逻辑语言的相继式演算的完备性(completeness)：任何$\Phi$与$\varphi$，只要$\Phi \models \varphi$成立，就有$\Phi \vdash \varphi$成立。
...
>
> 同理，$\Phi \models \neg\varphi$和$\Phi\not\models \varphi$的含义也截然不同。前者的含义是，对于任意$\I$，只要$\I(\Phi)=true$，就有$\I(\varphi)=false$；后者的含义是，存在$\I$使得$\I(\Phi)=true$而$\I(\varphi)=false$。在绝大多数情况下，二者是不等价的。
...
- 原子性的情况，$\I^{\Phi}(x)=\bar x$与$\I^\Phi(c)=\bar c$根据定义已经成立；
...
- 当$\varphi=t_1\equiv t_2$时，我们已经确认$\Phi \vdash \varphi \implies \I^\Phi(\varphi)=true$确实成立；
- 当$\varphi = Rt_1\cdots t_n$时，$\Phi \vdash Rt_1\cdots t_n$当且仅当$R^\Phi(\bar t_1,\cdots,\bar t_n)$成立当且仅当$\It(Rt_1\cdots t_n)=true$。
...
考虑$S=\{R\}$，$\Phi=\{Rx \lor Ry\}$。那么$\Phi \vdash Rx\lor Ry$，我们想要推出$\It (Rx\lor Ry)=true$，也即“$\It(Rx)=true$或$\It(Ry)=true$”。假如$\It (Rx)=true$成立，这等价于$\Phi \vdash Rx$，由可靠性得$\Phi \models Rx$。此时可以构造$\I_1$：论域为自然数$\{0,1\}$，$R(0)$成立$R(1)$不成立，$\beta(x)=1,\beta(y)=0$，那么$\I_1(Rx\lor Ry)=true$但是$\I_1(Rx)=false$，这说明$\Phi \not\models Rx$，矛盾。同理，$\It(Ry)=true$也不成立。说明$\It (Rx\lor Ry)=false$。这里的问题出自“或”这一连接词，它导致我们不能推出由“或”连接的任意一个子formula究竟是真是假。例如上面我们已经证明了$\Phi \vdash Rx$不成立，还可以证明$\Phi \vdash \neg Rx$也不成立：如果$\Phi \vdash \neg Rx$，那么$\Phi \models \neg Rx$，可以构造一个$\I_1'$令universe为自然数$\{0,1\}$，$R(0)$成立$R(1)$不成立，$\beta(x)=0,\beta(y)=1$，那么$\I_1'(\Phi)=true$但$\I_1'(\neg Rx)=false$，矛盾。换言之，对于$\Phi$存在一个命题$\varphi$使得$\Phi \vdash \varphi$与$\Phi \vdash \neg \varphi$都不成立，这就使得我们无法对“或”连接词做归纳了：因为即便两个子命题的正面和反面都不可证，这两个子命题的“或”却是可证的。
...
- 左推右：假设$\It (\varphi \lor \psi)=true$，也即$\It (\varphi)=true$或$\It (\psi)=true$，由归纳假设这当且仅当$\Phi \vdash \varphi$或$\Phi \vdash \psi$。无论前者成立还是后者成立，都可以由规则七得$\Phi \vdash (\varphi \lor \psi)$。右推左：假设$\Phi \vdash (\varphi \lor \psi)$，假如$\Phi \vdash \varphi$成立，由归纳假设$\It (\varphi)=true$；假如$\Phi \vdash \varphi$不成立，由negation complete得到$\Phi \vdash \neg\varphi$必须成立，由Moified Or Rule推出$\Phi \vdash \psi$，由归纳假设$\It (\psi)=true$；
