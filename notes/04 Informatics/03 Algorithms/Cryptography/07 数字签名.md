...
考虑以下实验 $\text{SigForge}_{\mathcal{A}, \Pi}(n)$：输入 $n$，由 $\text{Gen}$ 生成 $(k_p,k_s)$；敌手 $\mathcal{A}$ 得到输入 $k_p$，询问签名 oracle 多项式次，得到许多二元组 $(m,\sigma)$；敌手最终输出一个不曾询问过 oracle 的消息 $m_0$ 和对应的签名 $\sigma_0$，如果通过认证 $\text{Vrfy}_{p_k}(m_0,\sigma_0)=1$，就称敌手成功。如果任何敌手成功的概率都满足 $\Pr[\text{SigForge}_{\mathcal{A},\Pi}(n)=1]\leq \text{negl}$，就称该方案是安全的。
