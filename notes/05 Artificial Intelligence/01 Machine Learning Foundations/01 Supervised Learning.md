...
> ### Laplace Smoothing
>
> 我们发现，在上面的基于朴素贝叶斯的文本分类例子中，假如字典中的某个单词在训练数据集中从未出现过，那么根据$\phi_{j\mid y=0},\phi_{j\mid y=1}$的公式它们都将等于0，代入$p(y_i=1\mid x_i)$就会出现$0/0$的情况——无法做出预测！为此，我们在$\phi$的分子上+1，分母上+2，这样就保证了分子分母恒不会取0。例如，现在$\phi_{j\mid y=1}'=\dfrac{1+\sum\limits_{i=1}^{n}\mathbb{1}[x_i^{(j)}=1\and y_i=1]}{2+\sum\limits_{i=1}^{n}\mathbb{1}[y_i=1]}$，这称为Laplace Smoothing。
