...
从数学上，如果我们设所有样本（图片）的集合为$\mathscr{X}$，标签集合为$\mathscr{Y}$，那么ground truth就是样本集$\X\times \Y$上的一个概率空间：

$$(\X\times \Y,\sigma(\X\times\Y),\mathbb{P})$$

> 注意，这样定义ground truth意味着“图片”本身也有一个概率分布。这其实预设了我们想要模型能解决怎么样的问题。例如，“猫的图片”在“手写数字识别”的ground truth下概率应当很小，因为我们并不期待模型能对猫的图片做出良好的预测。

## Training Set

可以看到，ground truth的分布只在理论上存在，我们永远无法获得其分布的具体值。因为如果非要弄清楚ground truth是什么，我们有必要对过去、现在、未来的世界上的全部人口做调查！因此，ground truth只是数学上的一个定义。在现实中，通常的做法是：选取一部分图片，并让一部分人为它们标注上标签。当图片足够多，且标注的人头脑正常，那么就可以把这些图片的标注的概率分布“近似看作”ground truth。这个带有标注的“图片-标注”集合就称为一个训练集(training set)。
...
根据训练集，我们要训练出用于预测的函数$f:\X\to \Y$，称为预测函数(prediction function)。我们通常把$f$的选择分为两个过程：第一步，选定一个基本的模型，这规定了一个$\X\to \Y$的函数类$\mathscr{F}$；第二步，调整模型的参数，也即找到$\mathscr{F}$中的某个“较好”的函数。
...
有了loss function以后，我们自然可以这样建模来评估$\X$上$h$的表现：考察从ground truth中采样(sample)的图像的正确标签和$h$预测的标签的loss的期望：$L(h)=\E_{(X,Y)\sim \P}[\ell(h(X),Y)]$，其中$L(h)$称为$h$的population risk(or expected loss)。函数集$\mathscr{H}$上最小的population risk是$\inf\limits_{g \in \mathscr{H}}L(g)$，所以我们把$E(h)=L(h)-\inf\limits_{g \in \mathscr{H}}L(g)$称为$h$的excess risk。

然而在寻找（训练）$h$的过程中，我们只能在事先采样好的训练集上评估$h$。训练时我们实际用来评估的是：$\hat L_n(h)=\dfrac{1}{n}\sum\limits_{i=1}^{n}\ell(h(x_i),y_i),(x_i,y_i)\in D$，其中$\hat L_n(h)$称为empirical risk(training loss)。训练的目的就是找到$\hat h^*$满足$\hat h^*=\arg\inf\limits_{h\in\mathscr{H}}\hat L_n(h)$，也即最小化empirical risk的函数$\hat h^*$。

注意到，因为训练样本本身也是独立同分布的从ground truth采样的，所以empirical risk的期望就是population risk：$\E_{(x_i,y_i)\stackrel{\text{i.i.d.}}{\sim}\P}[\hat L_n(h)]=\E_{(x_i,y_i)\stackrel{\text{i.i.d.}}{\sim}\P}\left[\dfrac{1}{n}\sum\limits_{i=1}^{n}\ell(h(x_i),y_i)\right]=\E_{(X,Y)\sim \P}[\ell(h(X),Y)]=L(h)$，也即empirical risk的估计与ground truth相比是不带偏差(bias)的。



## Empirical Risk Minimization

在机器学习中，通常我们先固定模型，然后调整模型的参数来最小化empirical risk。这个过程就称为empirical risk minimization。我们设函数$h$是关于参数$\theta$的，于是最小化empirical risk的任务就是找到$\hat \theta$满足$\hat \theta=\arg\min\limits_{\theta\in\Theta}\hat L(h_\theta)$$=\arg\min\limits_{\theta\in\Theta}\dfrac{1}{n}\sum\limits_{i=1}^{n}\ell(h_\theta(x_i),y_i),(x_i,y_i)\in D$。

我们已经知道，训练集越大（$n$越大），训练集内样本的分布就越接近ground truth。那么自然可以期待，训练集越大，empirical risk就会越接近population risk。用数学的语言来说，即当$n\to\infty$时$\hat L(h_\theta)\to L(h_{\theta^*})$。其中$\theta^*=\arg\min\limits_{\theta\in \Theta}L(h_\theta)$，也即在ground truth上最优的参数。

假设$\hat \theta\stackrel{p}{\to}\theta^*$，我们可以分析其收敛的速率：可以证明，$\sqrt{n}(\hat \theta-\theta^*)=O_P(1)$，$\sqrt{n}(\hat \theta-\theta^*)\stackrel{d}{\to}N(0,(\nabla^2L(\theta^*))^{-1}\text{Cov}(\nabla \ell(h_{\theta^*}(x),y))(\nabla^2L(\theta^*))^{-1})$。注意，其中$\hat\theta,\theta^*$都是随机变量，因为训练集和population risk都是从ground truth采样得到的。其中，$O_P(1)$是概率意义下的$O(1)$，其具体的数学含义为：一列随机变量$X_n$满足$X_n=O_P(1)$当且仅当$\forall\varepsilon>0,\exists M>0, \sup\limits_{n}(\mathbb{P}(|X_n|>M))<\varepsilon$。
