...
我们根据事件的独立性，可以定义随机变量的独立性。如果随机变量$X,Y$满足$\forall x,y \in \R$，$P(X \leq x \and  Y \leq y)=P(X \leq x)\cdot P(Y \leq y)$，就称随机变量$X,Y$独立。这个定义看似只对所有形如$(-\infty,a]$的区间定义，实际上它与“$\forall A,B \in \B(\R)$，$P(X \in A \and Y \in B)=P(X \in A)P(Y \in B)$”是等价的。这同样是源于$(-\infty,a]$与Borel Set的等价性，证明略。
