...
$\newcommand{\F}{\mathcal{F}}$Kolmogorov建立了一般概率空间的公理系统，这套系统是建立在测度论语言之上的。在这套系统中，我们用$\sigma$-algebra来定义事件集：对于样本集为$\Omega$，其事件集$\F$是$\Omega$上的一个$\sigma$-algebra。也即$\F$满足三个条件：
...
对于可测空间$(\Omega,\F)$，如果函数$P:\F\to [0,\infty]$是一个测度，并且满足$P(\Omega)=1$，就称$P$是一个概率测度。具体地，函数$P$满足下面三个条件：

- $P(\varnothing)=0$；
- $P(\Omega)=1$；
- 至多可数个互不相交的事件序列$A_1,A_2,\cdots$满足$P(\bigcup\limits_{i\geq 1} A_i)=\sum\limits_{i\geq 1} P(A_i)$

三元组$(\Omega,\F,P)$称为一个概率空间。容易验证，当$\Omega$为有限集，且$\F=2^\Omega$时，上述定义的概率空间等价于离散概率空间。
...
$\newcommand{\F}{\mathcal{F}}\newcommand{\B}{\mathcal{B}}\newcommand{\Var}{\text{Var}}\newcommand{\E}{\mathbb{E}}$在离散概率中，随机变量是样本集到实数的函数$X:\Omega \to \R$，任何一个函数$X:\Omega \to \R$都是一个随机变量。然而，在一般的概率空间中，并不是所有$\Omega\to\R$的映射都能被称为随机变量。其理由是：在离散概率中，我们常用到“$X=a$”或“$X\in I$”这样的事件，正是基于这样的事件我们定义了随机变量的分布、期望等等概念。所以这要求$X^{-1}(a)$，$X^{-1}(I)$必须落在$\sigma$-algebra $\F$内。于是，最自然的做法就是把随机变量定义为$\F$-可测函数。也即：

$X:\Omega \to \R$是随机变量，当且仅当$\forall A \in \B(\R)$，$X^{-1}(A) \in \mathcal{F}$。其中$\B(\R)$就是$\R$上的Borel Set。

因为$\B(\R)$可以等价定义为$\sigma(\{(-\infty,a]\mid a\in \R\})$或$\sigma(\{(-\infty,a]\mid a\in \Q\})$，所以要验证$X$是随机变量，只需验证所有形如$(-\infty,a]$的区间的原像是否在$\F$中。

> 给定随机变量$X$，我们知道$\forall A\in \B(\R)$都有$X^{-1}(A)\in \F$。因此我们可以定义一个函数$\mu_X:\B(\R)\to [0,1]$，其中$\mu_X(A):=P(X\in A)$。我们来验证，$(\R,\B(\R),\mu_X)$是一个测度空间。$\mu_X(\varnothing)=P(\varnothing)=0$；对于两两无交的集合$\{E_i\}\in \B(\R)$，$\mu_X(\bigcup\limits_{i=1}^{\infty}E_i)=P(X\in \bigcup\limits_{i=1}^{\infty}E_i)=\sum\limits_{i=1}^{\infty}P(X\in E_i)$$=\sum\limits_{i=1}^{\infty}\mu_X(E_i)$。由此可见，每个随机变量$X$都给出了一个对应的$\B(\R)$上的测度，这样我们就得到了许多$\B(\R)$上的完全不同于勒贝格测度的测度。
