...
随机变量的另一个重要信息是方差，定义为$\Var(X)=\E[(X-\E[X])^2]$或$\Var(X)=\E[X^2]-\E[X]^2$。方差的含义可以Chebyshev不等式解释清楚，这本质是把二次函数应用到Markov不等式上：对于随机变量$X$（不一定非负）以及非负的实数$a$，成立$\Pr[|X-\E [X]|\geq a]\leq \dfrac{\Var(X)} {a^2}$。证明如下：$\Pr[|X-\E [X]|\geq a]=$ $\Pr[(X-\E [X])^2\geq a^2]$，随机变量$(X-\E [X])^2$非负，由Markov不等式可得$\Pr[(X-\E [X])^2\geq a^2]\leq \dfrac{\E [(X-\E [X])^2]}{a^2}=\dfrac{\Var(X) }{a^2}$，证毕。其中，随机变量分布于期望两边距离$a$以上的概率总和$\Pr[|X-\E[X]|\geq a]$也可以看作tail。Chebyshev不等式告诉我们，这个tail能被一个正比于方差的值bound住。方差越小，tail就越小，随机变量的分布就越集中在期望附近。可见方差可以衡量随机变量的分布在期望附近的集中情况。
...
>   \Pr[X\geq (1-\delta)\mu]\leq e^{-\frac{1}{2}\delta^2\mu}
> \end{matrix}\right.$
...
和Chernoff Bound中类似，我们寻找另外的能够bound $\E[e^{tX}]$的情况。一个可行的情况是$X$是$n$个独立的属于某个闭区间的随机变量的和（不一定同分布）。那么可以设$X_i\in [a_i,b_i]$，$X=\sum\limits_{i=1}^{n}X_i$，那么$\E [e^{tX}]=\E [e^{t\sum\limits_{i=1}^{n}X_i}]$ $=\E [\prod\limits_{i=1}^{n}e^{tX_i}]$$=\prod\limits_{i=1}^{n}\E [e^{tX_i}]$。我们可以不失一般性假设$X_i$的期望都是$0$，那么$a_i<0<b_i$，此时有Hoeffding Lemma指出$\E[e^{tX_i}]\leq e^{\frac{t^2(b_i-a_i)^2}{8}}$。这是一个凸不等式，证明比较复杂，此处省略。于是$\prod\limits_{i=1}^{n}\E [e^{tX_i}]\leq\prod\limits_{i=1}^{n}e^{\frac{t^2(b_i-a_i)^2}{8}}=e^{\frac{t^2}{8}\sum\limits_{i\in [n]}(b_i-a_i)^2}$。所以我们代入$\Pr[X\geq a]=\Pr[e^{tX}\geq e^{ta}]\leq\dfrac{\E [e^{tX}]}{e^{ta}}$得到$\Pr[X\geq a]\leq \exp\left(\dfrac{t^2}{8}\sum\limits_{i\in [n]}(b_i-a_i)^2-ta\right)$，取二次函数最小值点$t=\dfrac{4a}{\sum\limits_{i\in [n]}(b_i-a_i)^2}$代入，得到$\Pr[X\geq a]\leq \exp\left(-\dfrac{2a^2}{\sum\limits_{i\in[n]}(b_i-a_i)}\right)$。另一侧也是同理，$\Pr[X\leq -a]\leq \exp\left(-\dfrac{2a^2}{\sum\limits_{i\in[n]}(b_i-a_i)}\right)$。

综上，我们有Hoeffding不等式：对于一列独立的随机变量$X_i$，如果$X_i\in [a_i,b_i]$，那么设$X=\sum\limits_{i=1}^{n}X_i$，那么有$\Pr[|X-\E[X]|\geq t]\leq 2 \exp\left({-\dfrac{2t^2}{\sum\limits_{i\in[n]}(b_i-a_i)}}\right)$。
