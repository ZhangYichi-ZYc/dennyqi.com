...
下面证明，存在$(\Omega,\F,P)$上的随机变量$X$和随机变量列$X_1,X_2,\cdots$，$X_n \stackrel{p}{\to} X$成立，而$X_n \stackrel{a.s.}{\to} X$不成立。反例：令$\Omega=[0,1],\F=\B([0,1]),P$为勒贝格测度。令$X$为全零函数。$\forall n \geq 1$，设$m = \lceil \log_2(n+1) \rceil - 1$，对于$k\in [1, 2^m]$，定义$X_n = 
\begin{cases}
...
一般而言，$X_n \stackrel{a.s.}{\to} X$与$X_n \stackrel{L^1}{\to} X$的关系是不可比较的。左推右的反例：$X_n=n \cdot \mathbb{1}_{[\frac{1}{n},\frac{2}{n}]}$，$X\equiv 0$，其中$\E[|X_n-X|]=\E[X_n]=1$。右推左的反例：$X_n = 
\begin{cases}
1, & \omega \in [(k-1)\cdot 2^{-m}, k\cdot 2^{-m}) \\
0, & \text{otherwise}.
...
如果事件序列$\{E_n\}$是mutually independent(相互独立)的，那么Borel-Cantelli第一引理的逆命题成立，这称为Borel-Cantelli第二引理：在测度空间$(\Omega,\F,P)$上，对于一列mutually independent的事件$E_1,E_2,\cdots$，如果$P(\overline{\lim}\limits_{n\to\infty}E_n)=0$，那么$\sum\limits_{n=1}^{\infty}P(E_n)<\infty$。
...
设$\{X_n\}$是概率空间$(\Omega,\F,P)$上一列独立且同分布(independent and  identically distributed, i.i.d.)的随机变量，定义$S_n:=\sum\limits_{i =1}^{n}X_i$，那么$\{\dfrac{S_n}{n}\}$也是一列$(\Omega,\F,P)$上的随机变量。下面这个结论称为Khinchin弱大数定律：如果$\E[X_1]<\infty$，则$\dfrac{S_n}{n}\stackrel{p}{\to}\E[X_1]$。

> i.i.d中的“independent”是指“mutually independent”。

注意，因为$\{X_n\}$是独立同分布的，所以$\E[X_1]<\infty$这个条件指的其实是$\forall i\in \N,\E[X_i]<\infty$，因为所有$\E[X_i]$都是同一个值。同理，$\dfrac{S_n}{n}\stackrel{p}{\to}\E[X_1]$中的$\E[X_1]$也可以是任何一个$\E[X_i]$。注意，$\dfrac{S_n}{n}\stackrel{p}{\to}\E[X_1]$中的$\E[X_1]$是指一个常值随机变量而不是一个实数，严格的写法应该是：令$Y$满足$\forall \omega\in \Omega,Y(\omega)=\E[X_1]$，有$\dfrac{S_n}{n}\stackrel{p}{\to}Y$。

在证明Khinchin弱大数定律之前，我们应该指出这样一件事。如果我们在Khinchin弱大数定律的前提中附加上一个“二阶矩有界”条件，那么证明会变得很容易：存在$\sigma\in \R$，使得$\E[X_1^2]<\sigma^2$。有了这个条件以后，证明只需要用一次Chebyshev不等式：要证$\dfrac{S_n}{n}\stackrel{p}{\to}\E[X_1]$，即证$\forall \varepsilon>0$，$\lim\limits_{n\to\infty}P\left(\left|\dfrac{S_n}{n}-\E[X_1]\right|>\varepsilon\right)=0$。对于任意$n\in \N$，我们有$\E\left[\dfrac{S_n}{n}\right]=\dfrac{1}{n}\sum\limits_{i=1}^{n}\E[X_i]=\dfrac{1}{n}\cdot n\E[X_1]=\E[X_1]$，$\Var\left[\dfrac{S_n}{n}\right]=\dfrac{1}{n^2}\sum\limits_{i=1}^{n}\Var[X_i]=\dfrac{\Var[X_1]}{n}=\dfrac{\E[X_1^2]-\E[X_1]^2}{n}\leq \dfrac{\sigma^2}{n}$。所以$P\left(\left|\dfrac{S_n}{n}-\E[X_1]\right|>\varepsilon\right)=P\left(\left|\dfrac{S_n}{n}-\E\left[\dfrac{S_n}{n}\right]\right|>\varepsilon\right)\leq \dfrac{\Var\left[\dfrac{S_n}{n}\right]}{\varepsilon^2}\leq\dfrac{\sigma^2}{n\varepsilon^2}$。于是$\lim\limits_{n\to\infty}P\left(\left|\dfrac{S_n}{n}-\E[X_1]\right|>\varepsilon\right)\leq\lim\limits_{n\to\infty}\dfrac{\sigma^2}{n\varepsilon^2}=0$，证毕。

从上面的证明可以看到，我们只用到了$\{X_n\}$的独立性，而没有用到关于同分布的任何条件。同时，$\E[X_1^2]<\sigma^2$的条件也可以取代$\E[X_1]<\infty$的条件。所以我们实际得出了这样一个结论，这可以看作弱大数定理的另一种形式：设$\{X_n\}$是概率空间$(\Omega,\F,P)$上一列mutually independent的随机变量，令$S_n=\sum\limits_{i=1}^{n}X_i$，如果存在$\sigma\in \R$使得$\E[X_1^2]<\sigma^2$，则$\dfrac{S_n}{n}\stackrel{p}{\to}\E[X_1]$。

现在我们来看如何证明Khinchin弱大数定律。上面的证明的关键在于利用$X_1$的二阶矩的上界，由Chebyshev不等式给出了$P\left(\left|\dfrac{S_n}{n}-\E[X_1]\right|>\varepsilon\right)$的上界。但是现在如果只知道$X_1$的一阶矩有界，我们就无法再直接用concentration不等式来令上界趋向于$0$。比如，我们可以假设$X_1$非负，尝试Markov不等式，则$P\left(\dfrac{S_n}{n}-\E[X_1]>\varepsilon\right)=P\left(\dfrac{S_n}{n}>\E[X_1]+\varepsilon\right)\leq\dfrac{\E[X_1]}{\E[X_1]+\varepsilon}$，无法成功。

为了证明Khinchin弱大数定律，我们可以采用一种称为“截断法(truncation)”的证明技巧。对于随机变量$X_i$，我们把它分解为两个随机变量之和，其中一个是$X_i$取值不超过上界$M$的部分，另一个是剩余部分。这样，前一部分的随机变量就有了用二阶矩有界的性质，而通过选取恰当的$M$（可以是一个关于$n$的值），我们可以使得第二部分随机变量被取到的概率趋向$0$，这样就完成了证明。具体地，令$X_i^{\leq M}(\omega):=X_i(\omega)\cdot \mathbb{1}[|X_i(\omega)|\leq M]$，$X_i^{>M}(\omega):=X_i(\omega)\cdot \mathbb{1}[|X_i(\omega)|>M]$，那么$X_i=X_i^{\leq M}+X_i^{>M}$。于是$S_n=\sum\limits_{i=1}^{n}(X_i^{\leq M}+X_i^{>M})$，记$S_n^{\leq M}:=\sum\limits_{i=1}^{n}X_i^{\leq M}$，$S_n^{>M}:=\sum\limits_{i=1}^{n}X_i^{>M}$，那么$S_n=S_n^{\leq M}+S_n^{>M}$。对于任意$\varepsilon>0$，$P\left(\left|\dfrac{S_n}{n}-\E[X_1]\right|>\varepsilon\right)=P\left(\left|\dfrac{S_n^{\leq M}}{n}-\E[X_1]+\dfrac{S_n^{> M}}{n}\right|>\varepsilon\right)$，其中，$P\left(\left|\dfrac{S_n^{\leq M}}{n}-\E[X_1]+\dfrac{S_n^{> M}}{n}\right|>\varepsilon\right)=P\left(\left|\dfrac{S_n^{\leq M}}{n}-\E[X_1]+\dfrac{S_n^{> M}}{n}\right|>\varepsilon\land S_n^{>M}=0\right)+$$P\left(\left|\dfrac{S_n^{\leq M}}{n}-\E[X_1]+\dfrac{S_n^{> M}}{n}\right|>\varepsilon\land S_n^{>M}\neq 0\right)\leq P\left(\left|\dfrac{S_n^{\leq M}}{n}-\E[X_1]\right|>\varepsilon\right)+P(S_n^{>M}\neq 0)$。

现在我们只需选取一个合适的$M$，使得$\lim\limits_{n\to\infty}P\left(\left|\dfrac{S_n^{\leq M}}{n}-\E[X_1]\right|>\varepsilon\right)=0$和$\lim\limits_{n\to\infty}P(S_n^{>M}\neq 0)=0$同时成立。显然$M$不能取常数，因为只要有一个$X_i$使得$P(X_i^{>M}\neq 0)>0$，根据$X_i$是独立同分布的，有$P(S_n^{>M}\neq 0)\geq P(X_1^{>M}\neq 0)$，于是$\lim\limits_{n\to\infty}P(S_n^{>M}\neq 0)\geq P(X_1^{>M}\neq 0)>0$。所以，无论$M$取什么常数，只要取满足$P(X_1^{>M}\neq 0)>0$的$X_1$，截断法就会失效。所以$M$必须取一个关于$n$的值。下面我们证明，为使得截断法有效，只需取$M=n$。

$\newcommand{\ep}{\varepsilon}$考虑$P\left(\left|\dfrac{S_n^{\leq n}}{n}-\E[X_1]\right|>\varepsilon\right)$这一项。由于做了截断，$\E\left[\dfrac{S_n^{\leq n}}{n}\right]=\E[X_1^{\leq n}]\neq\E[X_1]$，因此不能直接用Chebyshev不等式。但是，如果令$n\to\infty$，那么随机变量列$X_1^{\leq n}$点态收敛到$X_1$。而显然$|X_1^{\leq n}|<|X_1|$，而$\E[X_1]<\infty$，因此由控制收敛定理可得$\lim\limits_{n\to\infty}\E[X_1^{\leq n}]=\E[X_1]$。现在，要证明$\lim\limits_{n\to\infty}P\left(\left|\dfrac{S_n^{\leq n}}{n}-\E[X_1]\right|>\varepsilon\right)=0$。注意到，如果$\left|\dfrac{S_n^{\leq n}}{n}-\E[X_1]\right|>\varepsilon$成立，那么$\left|\dfrac{S_n^{\leq n}}{n}-\E[X_1^{\leq n}]\right|+|\E[X_1^{\leq n}]-\E[X_1]|>\varepsilon$成立，所以“$\left|\dfrac{S_n^{\leq n}}{n}-\E[X_1^{\leq n}]\right|>\varepsilon/2$或$|\E[X_1^{\leq n}]-\E[X_1]|>\ep/2$”成立。因此$P\left(\left|\dfrac{S_n^{\leq n}}{n}-\E[X_1]\right|>\varepsilon\right)\leq P\left(\left|\dfrac{S_n^{\leq n}}{n}-\E[X_1^{\leq n}]\right|>\varepsilon/2\right)+P(|\E[X_1^{\leq n}]-\E[X_1]|>\ep/2)$。因此，只需证$\lim\limits_{n\to\infty}P\left(\left|\dfrac{S_n^{\leq n}}{n}-\E[X_1^{\leq n}]\right|>\varepsilon/2\right)=0$以及$\lim\limits_{n\to\infty}P(|\E[X_1^{\leq n}]-\E[X_1]|>\ep/2)=0$。其中，后者是显然的，因为对于任意固定的$n$，$|\E[X_1^{\leq n}]-\E[X_1]|$是一个常数，因此当$n$足够大时总是有$P(|\E[X_1^{\leq n}]-\E[X_1]|>\ep/2)=0$。而证明$\lim\limits_{n\to\infty}P\left(\left|\dfrac{S_n^{\leq n}}{n}-\E[X_1^{\leq n}]\right|>\varepsilon/2\right)=0$就可以用Chebyshev不等式了：$P\left(\left|\dfrac{S_n^{\leq n}}{n}-\E[X_1^{\leq n}]\right|>\varepsilon/2\right)\leq\dfrac{\Var[X_1^{\leq n}]}{n\ep^2/4}$$\leq\dfrac{\E[(X_1^{\leq n})^2]}{n\ep^2}\leq \dfrac{1}{\ep^2}\cdot \E\left[\dfrac{(X_1^{\leq n})^2}{n}\right]$。因此$\lim\limits_{n\to\infty}P\left(\left|\dfrac{S_n^{\leq n}}{n}-\E[X_1^{\leq n}]\right|>\varepsilon/2\right)\leq$$\dfrac{1}{\ep^2}\cdot \lim\limits_{n\to\infty}\E\left[\dfrac{(X_1^{\leq n})^2}{n}\right]$。根据定义，$|X_1^{\leq n}|\leq n$，因此$\left|\dfrac{(X_1^{\leq n})^2}{n}\right|=\left|\dfrac{X_1^{\leq n}}{n}\right|\cdot \left|X_1^{\leq n}\right|\leq |X_1^{\leq n}|\leq |X_1|$。可见$\dfrac{(X_1^{\leq n})^2}{n}$能被$X_1$控制，所以由控制收敛定理，$\lim\limits_{n\to\infty}\E\left[\dfrac{(X_1^{\leq n})^2}{n}\right]=\E\left[\lim\limits_{n\to\infty}\dfrac{(X_1^{\leq n})^2}{n}\right]$。对于任意$\omega$，当$n$充分大时都有$\dfrac{(X_1^{\leq n}(\omega))^2}{n}=\dfrac{(X_1(\omega))^2}{n}$，因此$\lim\limits_{n\to\infty}\dfrac{(X_1^{\leq n}(\omega))^2}{n}=0$。由此可得$\E\left[\lim\limits_{n\to\infty}\dfrac{(X_1^{\leq n})^2}{n}\right]=0$。综上可知$\lim\limits_{n\to\infty}P\left(\left|\dfrac{S_n^{\leq n}}{n}-\E[X_1^{\leq n}]\right|>\varepsilon/2\right)=0$，因此$\lim\limits_{n\to\infty}P\left(\left|\dfrac{S_n^{\leq n}}{n}-\E[X_1]\right|>\varepsilon\right)=0$。

最后只需证明$\lim\limits_{n\to\infty}P(S_n^{>n}\neq 0)=0$。对于任意$n$，如果“$S_n^{>n}\neq 0$”成立，那么“$\bigvee\limits_{i=1}^{n}X_i^{>n}\neq 0$”成立，因此由Union Bound可知$P(S_n^{>n}\neq 0)\leq\sum\limits_{i=1}^{n}P(X_i^{>n}\neq 0)$$=n\cdot P(X_1^{>n}\neq 0)$。其中，$P(X_1^{>n}\neq 0)=P(\{\omega\mid X_1(\omega)>n\})$$=P(X_1>n)=\E[\mathbb{1}[X_1>n]]$。所以，只需证$\lim\limits_{n\to\infty}\E[n\cdot \mathbb{1}[X_1>n]]=0$。注意到，恰好成立$|n\cdot \mathbb{1}[X_1>n]|\leq |X_1|$，所以由控制收敛定理可得$\lim\limits_{n\to\infty}\E[n\cdot \mathbb{1}[X_1>n]]=\E[\lim\limits_{n\to\infty}(n\cdot \mathbb{1}[X_1>n])]=0$，证毕。
...
上面的思考暗示我们，在这个游戏中，不能思考“单轮”的票价。因为对于任何设定“较高”的票价$T$，想要“玩一次就把票价赚回来”的概率是极小的。除非运气特别好，想要“赚回票价”至少得玩好多轮。于是，我们假设总共玩$n$轮，那么总票价为$nT$。用随机变量$X_i$表示第$i$轮游戏玩家赢得钱，记$S_n=\sum\limits_{i=1}^{n}X_i$。如果票价是合理的，那么$S_n$应当与$nT$“接近”，也即$T$应该与$\dfrac{S_n}{n}$“接近”。可见，这是一个与大数定律形式很相近的问题。然而，现在$\E[X_i]=\infty$，所以不能用大数定律给出$\dfrac{S_n}{n}$的极限。事实上，我们应该已经注意到，票价不应该是一个常数，而应该是一个关于轮数$n$的函数$T(n)$。$T(n)$应当关于$n$递增，因为玩的轮数越多，玩家赢得大钱的概率越高。那么随着$n$的增长，$\dfrac{S_n}{n}$和$T(n)$都在变化，那么应该如何定义“$\dfrac{S_n}{n}$接近$T(n)$”呢？

一个自然的想法是类比依概率收敛，希望可以找到$T(n)$的表达式，使得$\forall \varepsilon>0$，$\lim\limits_{n\to\infty}P\left(\left|\dfrac{S_n}{n}-T(n)\right|>\ep\right)=0$。但是事实证明这样做并不能成功。我们能够做到的是找到这样的实数函数$T(n),\ep(n),q(n),\delta(n)$，使得
$$
P\left(\left|\dfrac{S_n}{nT(n)}-q(n)\right|>\varepsilon(n)\right)\leq \delta(n)
$$
其中当$n\to\infty$时，$q(n)\to 1,\ep(n)\to 0,\delta(n)\to 0$。让我们来理解这一定义。当$n$比较小的时候，$\dfrac{S_n}{nT(n)}$具有较大的随机性；而当$n$很大时，$\dfrac{S_n}{nT(n)}$的分布几乎集中在“$1$”这一点处。如果能找到这样的$T(n)$，我们就可以认为$nT(n)$是对$n$轮票价的一个合理估计。

用截断法。方便起见，不妨设$M=2^m$，$m$是一个正整数。那么$\E\left[\dfrac{S_n^{\leq 2^m}}{n}\right]=\E[X_1^{\leq 2^m}]$。根据游戏的定义，$X_1^{\leq 2^m}$对应“被截断到$m$轮的游戏”，因此$\E[X_1^{\leq 2^m}]=\sum\limits_{i=1}^{m}\dfrac{1}{2^i}\cdot 2^i=m$。同时，$\Var\left[\dfrac{S_n^{\leq 2^m}}{n}\right]=\dfrac{1}{n}\Var[X_1^{\leq 2^m}]$$\leq\dfrac{1}{n}\E[(X_1^{\leq 2^m})^2]=\dfrac{1}{n}\sum\limits_{i=1}^{m}\dfrac{1}{2^i}\cdot(2^i)^2=\dfrac{2^{m+1}-2}{n}\leq\dfrac{2^{m+1}}{n}$。由Chebyshev不等式，对任意$\ep$，$P\left(\left|\dfrac{S_n^{\leq 2^m}}{n}-m\right|>\ep\right)\leq\dfrac{2^{m+1}}{n\ep^2}$。仿照Khinchin弱大数定律中的论证，我们凑出：$P\left(\left|\dfrac{S_n}{n}-m\right|>\ep\right)\leq P\left(\left|\dfrac{S_n^{\leq 2^m}}{n}-m\right|>\ep\right)+P(S_n^{>2^m}\neq 0)$。我们有$P(S_n^{>2^m}\neq 0)\leq n\cdot P(X_1^{>2^m}\neq 0)=\dfrac{n}{2^m}$。这样我们就得到了$P\left(\left|\dfrac{S_n^{\leq 2^m}}{n}-m\right|>\ep\right)+P(S_n^{>2^m}\neq 0)\leq\dfrac{2^{m+1}}{n\ep^2}+\dfrac{n}{2^m}$。

把$\ep,m$都看作关于$n$的函数，我们需要$n\to\infty$时让$\dfrac{2^{m+1}}{n\ep^2}+\dfrac{n}{2^m}\to 0$。首先，为了使$\dfrac{n}{2^m}\to 0$，$m$的增长速率必须快于$\log_2 n$。但是这样就需要$\ep^2\to \infty$。事实上，我们不用担心这一点，因为这里的$\ep$并不是我们最终所求的$\ep(n)$，它只是截断法的一个中间结果。为了给再给$m$分配一部分增长速率，可以选择给它一个低于$\log_2 n$增长。令$m=\log_2 n+\dfrac{1}{2}\log_2\log_2 n$。这样，我们就需要用$\ep^2$抵消速率为$\log_2n$的增长，因此令$\ep=\sqrt{\log_2 n}$。于是我们得到这样一个结果：$P\left(\left|\dfrac{S_n}{n}-(\log_2 n+\dfrac{1}{2}\log_2\log_2 n)\right|>\sqrt{\log_2 n}\right)\leq \dfrac{2}{\sqrt{\log_2 n}}+\dfrac{1}{\sqrt{\log_2 n}}=\dfrac{3}{\sqrt{\log_2 n}}$。在概率的括号里两边同时除以$\log_2 n$，就得到：
$$
...
$$
于是，令$T(n)=\log_2 n,q(n)=1+\dfrac{\log_2\log_2 n}{2\log_2 n}$，$\ep(n)=\dfrac{1}{\sqrt{\log_2 n}}$，$\delta(n)=\dfrac{3}{\sqrt{\log_2 n}}$，就得到了我们想要的结果。由此可见，$n$轮票价的合理定价为每轮$\log_2 n$元。
