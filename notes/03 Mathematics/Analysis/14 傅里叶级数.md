...
$$
\dfrac{\partial T}{\partial t}=\alpha\cdot \dfrac{\part^2 T}{\partial x^2}
$$
...
因为该微分方程左右分别对不同的变量求导，所以容易配凑出满足该方程的解：$T(x,t)=\sin x\cdot e^{-\alpha t}$。检验：$\dfrac{\partial T}{\partial t}=-\alpha\sin x\cdot e^{-\alpha t}$，$\dfrac{\part^2 T}{\partial x^2}=-\sin x\cdot e^{-\alpha t}$，成立。

然而，上述微分方程只对细棒的内部成立。对于细棒的两个端点，该分析并不成立，因为端点处只受到一侧热传导。热力学的分析显示，端点处始终应当满足$\dfrac{\partial T}{\partial x}=0$。这意味着我们不能再简单地把$T(x,t)=\sin x\cdot e^{-\alpha t}$当作解。不过，我们可以平移函数$\sin x$至$\cos x$，再调整$\cos x$的周期为$L$。于是可以令
